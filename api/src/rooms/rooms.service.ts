...
import { PlayerStatus, RoomStatus, TransactionType } from 'generated/prisma/client';
...
const roomSelect = {
  ...
  finishedAt: true,
  createdAt: true,
  host: { select: { id: true, username: true, firstName: true, lastName: true, avatar: true } },
  players: {
    select: {
      id: true,
      status: true,
...
export class RoomsService {
  ...
  async findAll(query: QueryRoomsDto) {
    try {
      const where: any = { status: { not: RoomStatus.CANCELLED } };
  ...
  async findOne(id: string) {
    const room = await this.prisma.gameRoom.findUnique({ where: { id }, select: roomSelect });
  ...
  async create(dto: CreateRoomDto, userId: string) {
    try {
      const hashedPw = dto.password ? await bcrypt.hash(dto.password, 10) : undefined;
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        await tx.roomPlayer.create({ data: { roomId: room.id, userId, cardId: card.id } });
  ...
  async join(roomId: string, userId: string, dto: JoinRoomDto) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (!room) throw new NotFoundException('Room not found');
        if (room.status !== RoomStatus.WAITING) throw new BadRequestException('Room is not accepting players');

        const playerCount = await tx.roomPlayer.count({ where: { roomId } });
        if (playerCount >= room.maxPlayers) throw new BadRequestException('Room is full');

        const existing = await tx.roomPlayer.findUnique({ where: { roomId_userId: { roomId, userId } } });
        ...
        if (room.entryFee > 0) {
          ...
          await tx.user.update({ where: { id: userId }, data: { coinsBalance: { decrement: room.entryFee } } });
          await tx.gameRoom.update({ where: { id: roomId }, data: { prizePool: { increment: room.entryFee } } });
          ...
          if (wallet) {
            ...
            await tx.wallet.update({ where: { id: wallet.id }, data: { balance: { decrement: room.entryFee } } });
          }
        }
  ...
  async startGame(roomId: string, userId: string) {
    ...
    if (room.status !== RoomStatus.WAITING) throw new BadRequestException('Game already started');
    ...
    return this.prisma.$transaction(async (tx) => {
      await tx.roomPlayer.updateMany({ where: { roomId }, data: { status: PlayerStatus.PLAYING } });
  ...
  async claimBingo(roomId: string, userId: string) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        const room = await tx.gameRoom.findUnique({ where: { id: roomId } });
        if (!room || room.status !== RoomStatus.PLAYING) throw new BadRequestException('Game is not active');

        const player = await tx.roomPlayer.findUnique({ where: { roomId_userId: { roomId, userId } } });
        if (!player) throw new NotFoundException('Player not in room');
        if (player.hasBingo) throw new BadRequestException('Already claimed BINGO');
        ...
        await tx.gameRoom.update({
          where: { id: roomId },
          data: { status: RoomStatus.FINISHED, winnerId: userId, finishedAt: new Date() },
        });

        // credit prize
        await tx.user.update({ where: { id: userId }, data: { coinsBalance: { increment: prize } } });
        ...
        if (wallet && prize > 0) {
          ...
          await tx.wallet.update({ where: { id: wallet.id }, data: { balance: { increment: prize } } });
        }
