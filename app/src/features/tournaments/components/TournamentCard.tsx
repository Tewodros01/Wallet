...
export default function TournamentCard({
  ...
  const fill = Math.round((tournament.joinedCount / tournament.maxPlayers) * 100);
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          >
            ...
            <p className="text-xs font-black leading-tight text-white">{value}</p>
      ...
      <div className="flex flex-col gap-1">
        <div className="flex justify-between text-[10px] text-gray-500">
          ...
          <span>{tournament.maxPlayers - tournament.joinedCount} spots left</span>
        </div>
      ...
      {tournament.status !== "FINISHED" && tournament.status !== "CANCELLED" && (
        <button
          type="button"
          disabled={full || tournament.status === "LIVE" || tournament.isJoined}
        ...
        >
          {tournament.isJoined ? (
            ...
            <>Register — {tournament.entryFee === 0 ? "Free" : `${tournament.entryFee} coins`}</>
          )}
        </button>
      )}
    </div>
