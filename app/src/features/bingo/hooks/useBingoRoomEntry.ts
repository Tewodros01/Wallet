...
type UseBingoRoomEntryParams = {
  roomEntryId: string;
  userId?: string;
  onNeedCardSelection: (room: GameRoomDetail, maxCount: number) => Promise<void>;
};
...
export function useBingoRoomEntry({
  ...
  const resolveRoom = async (value: string) => {
    ...
    const localRoom = roomsData.find((room) => matchRoomLookup(room, normalized));
    ...
    try {
      ...
      return matchingRooms.find((room) => matchRoomLookup(room, normalized)) ?? null;
    }
  };
  ...
  const handlePaymentConfirm = async () => {
    ...
    setSelectedCardId(null);
    setPayRoom(null);
    await onNeedCardSelection(payRoom, payRoom.cardsPerPlayer);
    setScreen("card-select");
  };
  ...
  useEffect(() => {
    ...
    const enterFromRoute = async () => {
      ...
      try {
        ...
        const isJoined =
          room.players?.some((player: GameRoomPlayer) => player.user?.id === userId) ??
          false;
