      ...
      <div className="sticky top-0 z-40 bg-gray-950/95 backdrop-blur-xl border-b border-white/[0.06] px-5 pt-4 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-500">Pick up to 10 · Draw 20 of 80</p>
          </div>
        </div>
        ...
        {picked.size > 0 && phase === "pick" && (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {Object.entries(PAYOUTS[picked.size] ?? {}).map(([match, mult]) => (
                <div key={match} className="flex items-center justify-between bg-white/[0.04] rounded-xl px-3 py-2">
                  <span className="text-xs text-gray-400">{match} match{Number(match) > 1 ? "es" : ""}</span>
                  <div className="flex items-center gap-1">
                    <GiCoins className="text-yellow-400 text-[10px]" />
                    <span className="text-xs font-black text-yellow-300">{(mult * bet).toLocaleString()}</span>
                  </div>
                </div>
              ))}
            </div>
