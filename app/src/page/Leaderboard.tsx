        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500">Top players ranked by coins earned</p>
            </div>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`relative ${style.size} rounded-full ring-2 ${style.ring} overflow-hidden`}>
                        ...
                        <div className={`absolute -bottom-0.5 -right-0.5 w-5 h-5 ${style.bg} border border-white/20 rounded-full flex items-center justify-center`}>
                          <span className={`text-[9px] font-black ${style.text}`}>{rankNum}</span>
                        </div>
                      </div>
                      <div className="text-center">
                        <p className="text-xs font-black text-white leading-tight max-w-[72px] truncate">{p.user?.firstName ?? "—"}</p>
                        <div className="flex items-center justify-center gap-0.5 mt-0.5">
                          <GiCoins className="text-yellow-400 text-[9px]" />
                          <span className={`text-[10px] font-black ${style.text}`}>{fmt(p.totalEarned)}</span>
                        </div>
                      </div>
              ...
              {rest.map((p: any, i: number) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white truncate">{p.user?.firstName} {p.user?.lastName}</p>
                    <p className="text-[10px] text-gray-500">@{p.user?.username} · {p.wins}W</p>
                  </div>
            ...
            <>
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/25 rounded-2xl px-4 py-3.5 flex items-center gap-3">
                <div className="w-7 h-7 rounded-xl bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center shrink-0">
                  <span className="text-xs font-black text-emerald-400">#{myEntry.rank}</span>
                </div>
                <img src={myEntry.user?.avatar ?? "https://i.pravatar.cc/40"} alt="" className="w-9 h-9 rounded-full object-cover ring-2 ring-emerald-500/40 shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="text-sm font-black text-emerald-400 truncate">{myEntry.user?.firstName} {myEntry.user?.lastName}</p>
                    <span className="text-[9px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.5 rounded-full font-bold">YOU</span>
                  </div>
                  <p className="text-[10px] text-gray-500">@{myEntry.user?.username} · {myEntry.wins}W</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <GiCoins className="text-yellow-400 text-xs" />
                  <span className="text-xs font-black text-yellow-300">{fmt(myEntry.totalEarned)}</span>
                </div>
              </div>
            </>
          )}
