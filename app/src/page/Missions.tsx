...
export default function Missions() {
  ...
  const { mutate: claimMission, isPending, variables: claimingId } = useClaimMission();
  ...
  const handleClaim = (id: string) => {
    ...
    claimMission(id, {
      onSuccess: () => { fireConfetti(); play("win"); },
    });
  };
      ...
      <div className="sticky top-0 z-40 bg-gray-950/95 backdrop-blur-xl border-b border-white/[0.06] px-5 pt-5 pb-3 flex flex-col gap-3">
        ...
        <div className="flex gap-1.5 bg-white/[0.04] border border-white/[0.07] rounded-2xl p-1">
          {(["DAILY", "WEEKLY"] as const).map((t) => (
            <button key={t} type="button" onClick={() => { setTab(t); haptic.light(); }}
      ...
      <div className="flex flex-col gap-5 px-5 py-5 pb-10">
            ...
            {STREAK_DAYS.map((day, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-sm">{done ? "✅" : future ? "🔒" : "❌"}</span>
          ...
          {streak < 7 && (
            <div className="flex items-center gap-3 bg-black/20 rounded-2xl px-4 py-3">
              ...
              <div className="flex-1">
                <p className="text-xs font-bold text-white">Day {streak + 1} reward</p>
                <p className="text-[10px] text-gray-400">Play tomorrow to claim</p>
              </div>
              <div className="flex items-center gap-1">
                <FaCoins className="text-yellow-400 text-xs" />
                <span className="text-sm font-black text-yellow-300">+{STREAK_REWARDS[Math.min(streak, 6)]}</span>
              </div>
            </div>
          )}
        </div>
        ...
        {isLoading ? (
          <div className="flex flex-col gap-2.5">
            {[1, 2, 3, 4].map((i) => <SkeletonMissionCard key={i} />)}
          </div>
        ) : missions.length === 0 ? (
          <EmptyState type="missions" title="No missions available" subtitle="Check back later!" />
        ) : (
          <div className="flex flex-col gap-2.5">
            {missions.map((m, idx) => {
        ...
        {tab === "WEEKLY" && missions.length > 0 && (
          <div className="bg-white/[0.04] border border-white/[0.07] rounded-2xl p-4 grid grid-cols-3 gap-3">
            {[
              { icon: <FaTrophy className="text-yellow-400" />, label: "Completed", value: `${missions.filter((m) => m.claimed).length}/${missions.length}` },
              { icon: <FaCoins className="text-emerald-400" />, label: "Earned",    value: `${missions.filter((m) => m.claimed).reduce((s, m) => s + m.reward, 0)}` },
              { icon: <FaFire className="text-orange-400" />,   label: "Streak",    value: `${streak} days` },
