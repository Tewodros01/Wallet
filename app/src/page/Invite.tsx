...
import { FiArrowLeft, FiCheck, FiCopy, FiShare2, FiUserPlus, FiUsers } from "react-icons/fi";
import { useNavigate } from "react-router-dom";
import { AppBar } from "../components/ui/Layout";
import { useMyInvite, useAgentStats } from "../hooks/useAgents";
...
export default function Invite() {
  ...
  const inviteLink = invite ? `${window.location.origin}/signup?ref=${code}` : "";
  ...
  const handleShareLink = () => {
    if (navigator.share) {
      navigator.share({ title: "Join Bingo!", text: `Use my code ${code} to sign up!`, url: inviteLink });
    } else {
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col text-white">
      ...
      <div className="flex flex-col gap-5 px-5 py-5 pb-10">
        ...
        <div className="bg-gradient-to-br from-violet-500/10 to-purple-500/5 border border-violet-500/20 rounded-2xl p-5 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Earn <span className="text-yellow-300 font-bold">50 coins</span> for every user you invite who joins
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "Invited",  value: String(invitedUsers.length), color: "text-white"        },
            { label: "Active",   value: String(activeCount),          color: "text-emerald-400"  },
            { label: "Earned",   value: String(totalEarned),          color: "text-yellow-400"   },
          ].map(({ label, value, color }) => (
            <div key={label} className="bg-white/[0.04] border border-white/[0.07] rounded-2xl py-3 flex flex-col items-center gap-1">
              <span className={`text-lg font-black ${color}`}>{isLoading ? "—" : value}</span>
        ...
        <div className="flex flex-col gap-2">
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Your Referral Code</p>
          <div className="bg-white/[0.04] border border-white/[0.07] rounded-2xl px-4 py-3.5 flex items-center justify-between">
            <span className="text-xl font-black tracking-widest text-white">{isLoading ? "Loading..." : code}</span>
        ...
        {invitedUsers.length > 0 && (
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">People You Invited</p>
            ...
            <div className="bg-white/[0.04] border border-white/[0.07] rounded-2xl overflow-hidden">
              {invitedUsers.map((u: any, i: number) => (
                <div key={u.id} className={`flex items-center gap-3 px-4 py-3.5 ${i < invitedUsers.length - 1 ? "border-b border-white/[0.05]" : ""}`}>
                  <img src={u.avatar ?? `https://i.pravatar.cc/40?u=${u.id}`} alt={u.username} className="w-9 h-9 rounded-full object-cover ring-2 ring-white/10 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white">{u.firstName} {u.lastName}</p>
                    <p className="text-[11px] text-gray-500">@{u.username} · Joined {new Date(u.createdAt).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</p>
                  </div>
                  <span className="text-[10px] font-black px-2 py-0.5 rounded-full border bg-emerald-500/15 text-emerald-400 border-emerald-500/30">ACTIVE</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {invitedUsers.length === 0 && !isLoading && (
          <div className="flex flex-col items-center gap-3 py-8 text-gray-600">
            ...
            <p className="text-xs text-gray-700">Share your code to start earning!</p>
          </div>
        )}
      </div>
    </div>
