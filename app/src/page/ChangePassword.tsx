  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-3 bg-white/[0.06] border border-white/10 rounded-2xl px-4 py-3.5 focus-within:border-emerald-500 transition-all">
        ...
        <button type="button" onClick={() => setShow(!show)} className="text-gray-500 hover:text-gray-300 transition-colors shrink-0">
          {show ? <FiEyeOff /> : <FiEye />}
        </button>
      </div>
    </div>
...
function StrengthBar({ password }: { password: string }) {
  ...
  const textColors = ["", "text-rose-400", "text-orange-400", "text-yellow-400", "text-emerald-400"];
  if (!password) return null;
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-1.5">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className={`flex-1 h-1.5 rounded-full transition-all ${i <= score ? colors[score] : "bg-white/10"}`} />
        ))}
      </div>
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          {[{ label: "At least 8 characters", ok: checks[0] }, { label: "Uppercase letter", ok: checks[1] }, { label: "Number", ok: checks[2] }, { label: "Special character", ok: checks[3] }].map(({ label, ok }) => (
            <div key={label} className="flex items-center gap-1.5">
              <FiCheck className={`text-[10px] ${ok ? "text-emerald-400" : "text-gray-600"}`} />
              <span className={`text-[10px] ${ok ? "text-gray-400" : "text-gray-600"}`}>{label}</span>
            </div>
          ))}
        </div>
        <span className={`text-xs font-black ${textColors[score]}`}>{labels[score]}</span>
      </div>
    </div>
  );
}
...
export default function ChangePassword() {
  ...
  const canSubmit = current.length > 0 && newPass.length >= 8 && newPass === confirm;
  ...
  if (success) return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center gap-6 px-5 text-white">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-400 mt-1 text-sm">Your password has been updated. Please sign in again.</p>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col text-white">
      <AppBar
        ...
        right={<div className="w-8 h-8 rounded-xl bg-rose-500/15 border border-rose-500/25 flex items-center justify-center"><FiLock className="text-rose-400 text-sm" /></div>}
      />
      <div className="flex flex-col gap-5 px-5 py-6">
        ...
        {mismatch && <p className="text-xs text-rose-400 -mt-2">Passwords do not match</p>}
        {error && <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-3 text-xs text-rose-400 font-semibold">{error}</div>}
        <Button loading={isPending} disabled={!canSubmit} icon={<FiLock />} onClick={handleSave}>Update Password</Button>
      </div>
    </div>
  );
}
