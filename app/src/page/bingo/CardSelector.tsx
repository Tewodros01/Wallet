...
import { SAMPLE_CARDS } from "../../constance/bingoData";
...
export default function CardSelector({ card, onConfirm, onBack }: Props) {
  ...
  const cards = card ? [{ id: 0, board: card }] : SAMPLE_CARDS;
      ...
      <div className="flex flex-col gap-4 px-5 py-5 flex-1">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {card
              ? "This card was assigned to you by the server"
              : "Select one card before the game starts"}
          </p>
        </div>
