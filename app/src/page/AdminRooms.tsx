...
import { RoomStatus, type GameRoomDetail, type GameRoomPlayer } from "../types";
...
export default function AdminRooms() {
  ...
  const archivedRooms = rooms.filter(
    (room: GameRoomDetail) =>
      room.status === RoomStatus.FINISHED || room.status === RoomStatus.CANCELLED,
  );
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col gap-3">
              {visibleRooms.map((room: GameRoomDetail) => {
                const players = room._count?.players ?? room.players?.length ?? 0;
