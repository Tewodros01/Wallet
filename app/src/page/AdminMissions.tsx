...
import { FiArrowLeft, FiPlus, FiToggleLeft, FiToggleRight, FiTrash2, FiX } from "react-icons/fi";
...
const CATEGORIES = ["PLAY_GAMES", "WIN_GAMES", "DEPOSIT", "INVITE", "TOURNAMENT", "KENO"];
const TYPES = ["DAILY", "WEEKLY"];

function CreateModal({ onClose }: { onClose: () => void }) {
  const { mutate: create, isPending } = useCreateMission();
  const [form, setForm] = useState({
    title: "", desc: "", reward: "", total: "1",
    type: "DAILY", category: "PLAY_GAMES", icon: "🎯",
  });
    ...
    <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur-sm flex items-end justify-center">
      <div className="w-full max-w-md bg-gray-900 border border-white/10 rounded-t-3xl p-5 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-3">
          {[
            { key: "title", label: "Title *", placeholder: "e.g. Play 3 Games", type: "text" },
            { key: "desc", label: "Description *", placeholder: "e.g. Complete 3 bingo games today", type: "text" },
            { key: "reward", label: "Reward (coins) *", placeholder: "100", type: "number" },
            { key: "total", label: "Target Count", placeholder: "1", type: "number" },
            { key: "icon", label: "Icon (emoji)", placeholder: "🎯", type: "text" },
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wide mb-1">Type</p>
            ...
            <div>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wide mb-1">Category</p>
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button type="button" disabled={isPending || !form.title || !form.desc || !form.reward}
            onClick={() => create({ title: form.title, desc: form.desc, reward: parseInt(form.reward), total: parseInt(form.total) || 1, type: form.type, category: form.category, icon: form.icon || "🎯" }, { onSuccess: onClose })}
...
export default function AdminMissions() {
  ...
  const filtered = filter === "ALL" ? missions : missions.filter((m: any) => m.type === filter);
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col text-white">
      <AppBar
        left={
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-black text-white leading-tight">Missions</p>
              <p className="text-[10px] text-gray-500">{missions.length} total</p>
            </div>
          </div>
        }
        right={
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 pb-12 flex flex-col gap-4">
        ...
        {isLoading ? (
          ...
          filtered.map((m: any) => (
            <div key={m.id} className={`bg-white/[0.04] border rounded-2xl p-4 flex items-center gap-3 transition-all ${m.isActive !== false ? "border-white/[0.07]" : "border-white/[0.03] opacity-50"}`}>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button type="button"
                  onClick={() => update({ id: m.id, dto: { isActive: !(m.isActive !== false) } })}
                  className="text-lg text-gray-400 hover:text-emerald-400 transition-colors">
                  {m.isActive !== false ? <FiToggleRight className="text-emerald-400 text-xl" /> : <FiToggleLeft className="text-xl" />}
                </button>
