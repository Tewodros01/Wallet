    ...
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center gap-6 px-5 text-white">
      ...
      <div className="text-center">
        ...
        <div className="flex items-center justify-center gap-2 mt-1">
          ...
          <span className="text-5xl font-black text-yellow-300">{prize.coins}</span>
        </div>
        <p className="text-sm text-gray-400 mt-1">coins added to your wallet!</p>
      </div>
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col text-white">
      ...
      <div className="flex flex-col items-center gap-6 px-5 py-4">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <div className="relative w-72 h-72"
            ...
            <svg viewBox="0 0 200 200" className="w-full h-full drop-shadow-2xl">
              {PRIZES.map((p, i) => {
                const startAngle = i * SEGMENT_ANGLE - 90, endAngle = startAngle + SEGMENT_ANGLE;
                const s = Math.PI / 180;
                const x1 = 100 + 100 * Math.cos(startAngle * s), y1 = 100 + 100 * Math.sin(startAngle * s);
                const x2 = 100 + 100 * Math.cos(endAngle * s),   y2 = 100 + 100 * Math.sin(endAngle * s);
                const mx = 100 + 65 * Math.cos((startAngle + SEGMENT_ANGLE / 2) * s);
                const my = 100 + 65 * Math.sin((startAngle + SEGMENT_ANGLE / 2) * s);
                const tr = startAngle + SEGMENT_ANGLE / 2;
                return (
                  <g key={p.label}>
                    <path d={`M100,100 L${x1},${y1} A100,100 0 0,1 ${x2},${y2} Z`} fill={p.bg} stroke="#1f2937" strokeWidth="1" />
                    <text x={mx} y={my} textAnchor="middle" dominantBaseline="middle"
                      transform={`rotate(${tr}, ${mx}, ${my})`} fill={p.color} fontSize="11" fontWeight="900">{p.label}</text>
                  </g>
                );
              })}
              <circle cx="100" cy="100" r="18" fill="#111827" stroke="#374151" strokeWidth="2" />
              <text x="100" y="100" textAnchor="middle" dominantBaseline="middle" fill="#10b981" fontSize="14">🎱</text>
            </svg>
          </div>
          <div className="absolute inset-0 rounded-full pointer-events-none" style={{ boxShadow: "0 0 60px rgba(16,185,129,0.15)" }} />
        </div>
          ...
          <div className="w-full bg-white/[0.06] border border-white/[0.10] rounded-2xl p-4 flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-2xl font-black text-yellow-300">{prize.coins} coins</span>
              </div>
            </div>
        ...
        <div className="w-full flex flex-col gap-2">
          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Possible Prizes</p>
          <div className="grid grid-cols-4 gap-2">
            {PRIZES.map((p) => (
              <div key={p.label} className="rounded-xl py-2.5 flex flex-col items-center gap-1" style={{ background: p.bg, border: `1px solid ${p.color}30` }}>
