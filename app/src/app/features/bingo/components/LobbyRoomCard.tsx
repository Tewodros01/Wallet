    ...
    <div className="bg-white/4 border border-white/7 rounded-2xl p-4 flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <div className="relative shrink-0">
          {getPublicAssetUrl(room.host?.avatar) ? (
            <img
              src={getPublicAssetUrl(room.host?.avatar) ?? undefined}
              alt={room.host?.username}
              className="w-11 h-11 rounded-full object-cover ring-2 ring-white/10"
            />
          ) : (
            <div className="w-11 h-11 rounded-full bg-white/8 ring-2 ring-white/10 flex items-center justify-center text-white text-sm font-black">
              {getAvatarInitials(
                room.host?.firstName,
                room.host?.lastName,
                "?",
