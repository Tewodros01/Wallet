  ...
  ({ label, error, leftIcon, rightIcon, className = "", ...props }, ref) => (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative flex items-center">
        {leftIcon && (
          <span className="absolute left-3.5 text-gray-500 text-sm pointer-events-none">{leftIcon}</span>
        )}
        <input
          ref={ref}
        ...
        />
        {rightIcon && (
          <span className="absolute right-3.5 text-gray-500 text-sm">{rightIcon}</span>
        )}
      </div>
