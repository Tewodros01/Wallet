...
const CALL_SPEEDS: { label: string; sub: string; value: GameSpeed }[] = [
  { label: "Slow",   sub: "6s", value: "SLOW"   },
  { label: "Normal", sub: "4s", value: "NORMAL"  },
  { label: "Fast",   sub: "2s", value: "FAST"    },
];
...
function OptionPill({ label, sub, selected, onClick }: { label: string; sub?: string; selected: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick}
      ...
      {label}
      {sub && <span className={`text-[9px] font-normal ${selected ? "text-emerald-100" : "text-gray-600"}`}>{sub}</span>}
    </button>
  );
}

function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">{children}</p>;
}
...
export default function CreateRoomModal({ onClose, onEnter }: Props) {
  ...
  const [callSpeed,  setCallSpeed]  = useState<GameSpeed>("NORMAL");
  ...
  const roomLink = createdId ? `${window.location.origin}/game?room=${createdId}` : "";

  const handleCreate = () => {
    setError(null);
    createRoom(
      {
        name: roomName || `Room ${Date.now().toString(36).toUpperCase().slice(-4)}`,
        speed: callSpeed,
          ...
          {step === "settings" ? (
            <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-1.5">
                  {MAX_PLAYERS.map((p) => (
                    <OptionPill key={p} label={p} selected={maxPlayers === p} onClick={() => setMaxPlayers(p)} />
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-1.5">
                  {CARD_LIMITS.map((c) => (
                    <OptionPill key={c} label={c} sub={c === "1" ? "Basic" : c === "5" ? "Max" : ""} selected={cardLimit === c} onClick={() => setCardLimit(c)} />
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-1.5">
                  {CALL_SPEEDS.map(({ label, sub, value }) => (
                    <OptionPill key={value} label={label} sub={sub} selected={callSpeed === value} onClick={() => setCallSpeed(value)} />
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white/[0.04] border border-white/[0.07] rounded-2xl overflow-hidden">
                {[
                  {
                    icon: isPrivate ? <FaLock className="text-orange-400" /> : <FaUnlock className="text-emerald-400" />,
                    label: "Private Room",
                    sub: isPrivate ? "Only invited players can join" : "Anyone can browse and join",
                    on: isPrivate, toggle: () => setIsPrivate(!isPrivate),
                  },
              ...
              <div className="bg-white/[0.04] border border-white/[0.07] rounded-2xl p-4 grid grid-cols-2 gap-3">
                {[
                  { label: "Entry Fee",    value: entryFee === "Free" ? "Free" : `${entryFee} coins`, icon: <FaCoins className="text-yellow-400" /> },
                  { label: "Max Players",  value: maxPlayers,                                          icon: <FiUsers className="text-blue-400" /> },
                  { label: "Cards/Player", value: cardLimit,                                           icon: <FaGamepad className="text-violet-400" /> },
                  { label: "Call Speed",   value: callSpeed,                                           icon: <MdTimer className="text-orange-400" /> },
              ...
              <Button loading={isPending} icon={<FaGamepad />} size="lg" onClick={handleCreate}>
                Create Room
              </Button>
            </div>
          ) : (
            <div className="flex flex-col gap-5">
              <div className="bg-gradient-to-br from-emerald-500/10 to-teal-500/5 border border-emerald-500/20 rounded-2xl p-5 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Room ID</p>
                    <p className="text-lg font-black text-white tracking-widest">{createdId?.slice(-8).toUpperCase()}</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { label: "Entry",   value: entryFee === "Free" ? "Free" : entryFee },
                    { label: "Players", value: `0/${maxPlayers}` },
                    { label: "Cards",   value: cardLimit },
                  ].map(({ label, value }) => (
                    <div key={label} className="bg-white/[0.06] rounded-xl py-2 flex flex-col items-center gap-0.5">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2 bg-white/[0.05] border border-white/10 rounded-xl px-4 py-3">
                  <span className="text-xs text-gray-400 flex-1 truncate">{roomLink}</span>
              ...
              <div className="flex items-center gap-3 bg-orange-500/10 border border-orange-500/20 rounded-2xl px-4 py-3">
                ...
                <span className="text-xs font-black text-orange-400 ml-auto">0/{maxPlayers}</span>
              </div>

              <div className="flex gap-2">
                <Button variant="secondary" icon={<FiShare2 />} onClick={handleCopy}>Share Link</Button>
                <Button icon={<FaGamepad />} onClick={() => { onEnter?.(createdId!); onClose(); }}>Enter Room</Button>
              </div>
            </div>
          )}
        </div>
